...
int Block::GetTransactionCount(void) const
{
  return m_transactionCount;
}

void Block::SetTransactionCount(int transactionCount)
{
  m_transactionCount = transactionCount;
}
...
int Blockchain::GetNoOrphans(void) const
{
...
bool Blockchain::HasTransaction(const std::string transactionShortHash) const
{

  for (int height = 0; height < GetBlockchainHeight(); height++)
  {
    for (auto const &block_it : m_blocks[height])
    {
      ...
      {
        if (transactionShortHash == (it->second).GetTransactionShortHash())
        {
          return true;
        }
      }
...
void Blockchain::PrintOrphans(void)
{
  std::vector<Block>::iterator block_it;
  int orphanCount = 0;
...
int Blockchain::GetBlocksInForks(void)
{
...
int Blockchain::GetLongestForkSize(void)
{
  std::vector<std::vector<Block>>::iterator blockHeight_it;
  std::vector<Block>::iterator block_it;
...
const char *getMinerType(enum MinerType m)
{
  ...
  case SIMPLE_ATTACKER:
    return "SIMPLE_ATTACKER";
  case SELFISH_MINER:
    return "SELFISH_MINER";
  case SELFISH_MINER_TRIALS:
    return "SELFISH_MINER_TRIALS";
  }
}
